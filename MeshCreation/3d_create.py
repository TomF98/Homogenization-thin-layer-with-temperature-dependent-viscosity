...
############################################################
## Build reference cell with extra high resolution:
mesh_size_ref = 0.01
gmsh.clear()
...
coords = np.array(np.meshgrid(x_coords, y_coords)).T.reshape(-1, 2)
coords = np.column_stack((coords, rough_fn(eps*coords[:, :1], eps*coords[:, 1:])))
...
gmsh.model.addPhysicalGroup(3, [1], 1.0, name="Dummy")
